...
async def cmd_start(message: Message, lang: str = "uz"):
    ...
    text = (
        ...
        f"📊 Men istalgan bozorni tahlil qilaman:\n"
        f"• Forex, Crypto, Aksiyalar, Tovarlar\n\n"
        f"📸 Grafik skrinshot yuboring!\n\n{free_text}"
    )
...
async def cmd_help(message: Message, lang: str = "uz"):
    await message.answer(t(la
cd ~/workspace/trading_bot && cat > handlers/start.py << 'PYEOF'
from aiogram import Router, F
from aiogram.types import Message, CallbackQuery
from aiogram.filters import CommandStart, Command
from aiogram.utils.keyboard import InlineKeyboardBuilder
